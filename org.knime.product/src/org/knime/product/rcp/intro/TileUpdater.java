...
public class TileUpdater extends AbstractUpdater {
    ...
    private static final String WELCOME_PAGE_ENDPOINT = "https://www.knime.com/welcome-ap"; //$NON-NLS-1$
    ...
    protected void prepareData() throws Exception {
        ...
        if (TILE_CATEGORIES == null || IntroPage.MOCK_INTRO_PAGE) {
            try {
                ...
                LOGGER.info(Messages.getString("TileUpdater.1")); //$NON-NLS-1$
            }
        }
    }
    ...
    protected void updateData() {
        ...
        try {
            ...
            if (m_isFreshWorkspace) {
                hideElement("hub-search-bar"); //$NON-NLS-1$
            }
            updateTiles(tiles);
        } catch (IOException e) {
            LOGGER.error(Messages.getString("TileUpdater.3") + e.getMessage(), e); //$NON-NLS-1$
        }
    }
    ...
    private String buildTilesFromCategories() throws JsonProcessingException {
        ...
        for (int cat = 1; cat <= 3; cat++) {
            ...
            for (JSONCategory jsonCategory : TILE_CATEGORIES) {
                if (jsonCategory.getId().startsWith("c" + cat)) { //$NON-NLS-1$
            ...
            if (chosenCategoryTile == null || m_isFreshWorkspace) {
                try {
                    chosenCategoryTile = m_offlineCollector.fetchSingleOfflineTile("C" + cat, m_isFreshWorkspace); //$NON-NLS-1$
                } catch (IOException e) {
                    LOGGER.error(Messages.getString("TileUpdater.6") + e.getMessage(), e); //$NON-NLS-1$
    ...
    private URL buildTileURL(final Map<String, String> customizationInfo) {
        ...
        builder.append("?knid=" + KNIMEConstants.getKNIMEInstanceID()); //$NON-NLS-1$
        builder.append("&version=" + KNIMEConstants.VERSION); //$NON-NLS-1$
        builder.append("&os=" + Platform.getOS()); //$NON-NLS-1$
        builder.append("&osname=" + KNIMEConstants.getOSVariant()); //$NON-NLS-1$
        builder.append("&arch=" + Platform.getOSArch()); //$NON-NLS-1$

        // customization if present
        if (customizationInfo.containsKey("companyName")) { //$NON-NLS-1$
            //Add the customizers name to the URL
            String companyName = customizationInfo.get("companyName"); //$NON-NLS-1$
            if (StringUtils.isNoneEmpty(companyName)) {
                try {
                    builder.append("&brand=" + URLEncoder.encode(companyName, "UTF-8")); //$NON-NLS-1$ //$NON-NLS-2$
                } catch (UnsupportedEncodingException e) {
                    /* don't append customizer info */
                    LOGGER.warn(Messages.getString("TileUpdater.16") + e.getMessage(), e); //$NON-NLS-1$
                }
            }
        }

        // details
        builder.append("&details="); //$NON-NLS-1$
        builder.append(buildAPUsage());
        builder.append(","); //$NON-NLS-1$
        builder.append(buildHubUsage());

        try {
            return new URL(builder.toString().replace(" ", "%20")); //$NON-NLS-1$ //$NON-NLS-2$
        } catch (MalformedURLException e) {
            LOGGER.error(Messages.getString("TileUpdater.21") + e.getMessage(), e); //$NON-NLS-1$
            return null;
        }
    }

    private static String buildHubUsage() {
        String hubUsage = "hubUsage:"; //$NON-NLS-1$
        ...
        try {
            ...
            LOGGER.info(Messages.getString("TileUpdater.23") + e.getMessage(), e); //$NON-NLS-1$
        }

        if (lastUpload.isPresent()) {
            hubUsage += "contributer"; //$NON-NLS-1$
        } else if (lastLogin.isPresent()) {
            hubUsage += "user"; //$NON-NLS-1$
        } else {
            hubUsage += "none"; //$NON-NLS-1$
        }
        return hubUsage;
    }
    ...
    private String buildAPUsage() {
        ...
        String apUsage = "apUsage:"; //$NON-NLS-1$
        if (m_isFreshWorkspace) {
            apUsage += "first"; //$NON-NLS-1$
        } else {
            apUsage += "recurring"; //$NON-NLS-1$
        }
        return apUsage;
    }
    ...
    private void hideElement(final String id) {
        executeUpdateInBrowser("hideElement('" + id + "');"); //$NON-NLS-1$ //$NON-NLS-2$
    }

    private void updateTiles(final String tiles) {
        executeUpdateInBrowser("updateTile(" + tiles + ");"); //$NON-NLS-1$ //$NON-NLS-2$
    }
}
