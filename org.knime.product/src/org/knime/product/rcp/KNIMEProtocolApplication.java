...
public class KNIMEProtocolApplication implements IApplication {
    ...
    public Object start(final IApplicationContext context) throws Exception {
        ...
        File tmpFile = File.createTempFile("open-knime-link", ".knimeURL"); //$NON-NLS-1$ //$NON-NLS-2$
        try (PrintWriter writer = new PrintWriter(tmpFile, "UTF-8")) { //$NON-NLS-1$
            writer.write(stringArgs[0]);
        }
        Runtime.getRuntime().exec(stringArgs[1] + "\\knime.exe --launcher.openFile " + tmpFile.getPath()); //$NON-NLS-1$
    ...
    protected String[] retrieveApplicationArguments(final IApplicationContext context) {
        ...
        if (args instanceof String[]) {
            ...
            if (stringArgs.length > 0 && stringArgs[0].equals("-pdelaunch")) { //$NON-NLS-1$
                String[] copy = new String[stringArgs.length - 1];
                System.arraycopy(stringArgs, 1, copy, 0, copy.length);
                stringArgs = copy;
            }
        } else if (args != null) {
            System.err.println(Messages.getString("KNIMEProtocolApplication.5") + args.getClass().getName() //$NON-NLS-1$
                + Messages.getString("KNIMEProtocolApplication.6") + args.toString()); //$NON-NLS-1$
