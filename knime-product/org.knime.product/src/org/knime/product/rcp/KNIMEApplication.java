...
public class KNIMEApplication implements IApplication {
            ...
            if (display != null) {
                try {
                    ...
                    NodeLogger.getLogger(KNIMEApplication.class).debug(
                        org.knime.product.rcp.IDEWorkbenchMessages.KNIMEApplication_1 + e.getMessage(), e);
                }
            }
        }
    }
    ...
    private void parseApplicationArguments(final IApplicationContext context) {
        ...
        if (args instanceof String[]) {
            ...
            for (int i = 0; i < stringArgs.length; i++) {
                if ("-checkForUpdates".equals(stringArgs[i])) { //$NON-NLS-1$
                    m_checkForUpdates = true;
                }
            }
        } else if (args != null) {
            System.err
                    .println(org.knime.product.rcp.IDEWorkbenchMessages.KNIMEApplication_3
                            + args.getClass().getName()
                            + org.knime.product.rcp.IDEWorkbenchMessages.KNIMEApplication_4
                            + args.toString());
        }
    }
    ...
    private boolean checkInstanceLocation() {
        ...
        if (Platform.OS_MACOSX.equals(Platform.getOS())) {
            if (defaultLocation.getPath().contains("/Knime.app/")) { //$NON-NLS-1$
                URL url = Platform.getInstallLocation().getURL();
                try {
                    defaultLocation = new URL(url.getProtocol(), url.getHost(), url.getPath() + "/workspace"); //$NON-NLS-1$
        ...
        while (true) {
            ...
            try {
                ...
                MessageDialog.openError(null,
                                        "", //$NON-NLS-1$
                                        NLS.bind("", workspaceUrl.getPath())); //$NON-NLS-1$
            } catch (Exception e) {
                MessageDialog
                        .openError(
                                null,
    ...
    private URL promptForWorkspace(final ChooseWorkspaceData launchData, boolean force) {
        ...
        do {
            ...
            IDEWorkbenchMessages.ChooseWorkspaceDialog_dialogName = org.knime.product.rcp.IDEWorkbenchMessages.KNIMEApplication_7; //$NON-NLS-1$
            ...
            if (!workspace.exists() && !workspace.mkdir()) {
                ...
                MessageDialog.openError(null, "", //$NON-NLS-1$
                                        ""); //$NON-NLS-1$
                continue;
            }
            try {
    ...
    private boolean checkValidWorkspace(final URL url) {
        ...
        if (workspace_version > ide_version) {
            title = ""; //$NON-NLS-1$
            message = NLS.bind("", url.getFile()); //$NON-NLS-1$
        } else {
            title = ""; //$NON-NLS-1$
            message = NLS.bind("", url.getFile()); //$NON-NLS-1$
        }
    ...
    private boolean checkForUpdates() {
        ...
        if (provUI.getRepositoryTracker() == null) {
            LogHelper.log(new Status(IStatus.WARNING, ProductPlugin
                    ...
                    org.knime.product.rcp.IDEWorkbenchMessages.KNIMEApplication_8
                            + org.knime.product.rcp.IDEWorkbenchMessages.KNIMEApplication_9));
            return false;
        }
        ...
        if (agent == null) {
            LogHelper.log(new Status(IStatus.ERROR, ProductPlugin.getDefault()
                    ...
                    org.knime.product.rcp.IDEWorkbenchMessages.KNIMEApplication_10
                            + org.knime.product.rcp.IDEWorkbenchMessages.KNIMEApplication_11));
        }
            ...
            public void run(final IProgressMonitor monitor)
                ...
                SubMonitor sub =
                        SubMonitor.convert(monitor,
                                org.knime.product.rcp.IDEWorkbenchMessages.KNIMEApplication_12, 200);
                ...
                if (status.getSeverity() == IStatus.CANCEL) {
                    ...
                    if (status.getCode() == UpdateOperation.STATUS_NOTHING_TO_UPDATE) {
                        ...
                        LogHelper.log(new Status(IStatus.INFO, ProductPlugin
                                .getDefault().getBundle().getSymbolicName(),
                                org.knime.product.rcp.IDEWorkbenchMessages.KNIMEApplication_13));
                    } else if (status.getSeverity() != IStatus.ERROR) {
    ...
    private void startDeadlockDetectors(final Display display) {
        new GUIDeadlockDetector() {
            ...
            protected String getThreadName() {
                return "SWT Display thread"; //$NON-NLS-1$
            }
        ...
        new GUIDeadlockDetector() {
            @Override
            protected String getThreadName() {
                return "AWT Event Queue"; //$NON-NLS-1$
            }
