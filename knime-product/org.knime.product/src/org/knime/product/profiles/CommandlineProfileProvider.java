...
class CommandlineProfileProvider implements IProfileProvider {
    ...
    CommandlineProfileProvider() {
        ...
        for (int i = 0; i < args.length; i++) {
            if ("-profileList".equals(args[i])) { //$NON-NLS-1$
                if (i + 1 < args.length) {
                    m_requestedProfiles = Arrays.asList(args[++i].split("[,;:]")); //$NON-NLS-1$
                }
                // else ignore because there is no value
            } else if ("-profileLocation".equals(args[i])) { //$NON-NLS-1$
                if (i + 1 < args.length) {
                    try {
    ...
    public URI getProfilesLocation() {
        if (m_profilesLocation == null) {
            throw new IllegalArgumentException(Messages.CommandlineProfileProvider_3);
        }
