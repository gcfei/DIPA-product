...
public class WorkspaceProfileProvider implements IProfileProvider {
     ...
     */
    public static final String PROFILE_LOCATION = "profileLocation"; //$NON-NLS-1$

    /**
     * Constant for the name of the profile list property: {@value}.
     */
    public static final String PROFILE_LIST = "profileList"; //$NON-NLS-1$

    /**
     * Constant for the name of the property that tells whether profiles are enabled or not: {@value}.
     */
    public static final String PROFILES_ENABLED = "profilesEnabled"; //$NON-NLS-1$
    ...
    static {
        ...
        SETTINGS_FILE = stateDir.resolve("profile-settings.ini"); //$NON-NLS-1$
    }
    ...
    public WorkspaceProfileProvider() {
        try {
            ...
            Platform.getLog(myself).log(new Status(IStatus.ERROR, myself.getSymbolicName(),
                Messages.WorkspaceProfileProvider_4 + ex.getMessage(), ex));
        }
    }
    ...
    private void readWorkspaceSettings() throws IOException, URISyntaxException {
        if (Files.isRegularFile(SETTINGS_FILE)) {
            ...
            boolean enabled = Boolean.valueOf(props.getProperty(PROFILES_ENABLED, "false")); //$NON-NLS-1$
            ...
            if (enabled && (profileLocation != null)) {
                ...
                String requestedProfiles = props.getProperty(PROFILE_LIST, ""); //$NON-NLS-1$
                m_requestedProfiles = Arrays.asList(requestedProfiles.split("[,;:]")); //$NON-NLS-1$
            }
        }
    }
    ...
    public URI getProfilesLocation() {
        if (m_profilesLocation == null) {
            throw new IllegalArgumentException(Messages.WorkspaceProfileProvider_8);
        }
